...
public class Sql2oVacancyRepository implements VacancyRepository {
    ...
    public Vacancy save(Vacancy vacancy) {
        try (var connection = sql2o.open()) {
            var sql = """
                      INSERT INTO vacancies(title, description, creation_date, 
                      visible, city_id, file_id)
    ...
    public Optional<Vacancy> findById(int id) {
        try (var connection = sql2o.open()) {
            ...
            query.addParameter("id", id);
            var vacancy = query.setColumnMappings(
                    Vacancy.COLUMN_MAPPING).executeAndFetchFirst(Vacancy.class);
